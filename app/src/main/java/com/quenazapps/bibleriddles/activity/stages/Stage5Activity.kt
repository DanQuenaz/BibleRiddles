...
class Stage5Activity : ComponentActivity() {
    private val stageAnswers by lazy {
        StageAnswers(
            mainAnswer = getString(R.string.stage5_answer),
            alternativeAnswers = resources.getStringArray(R.array.stage5_accepted_answers).toList(),
        )
    }
    ...
    private var counter by mutableStateOf(Stage5CounterState())
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        counter = Stage5CounterState(
            side = Stage5Side.entries.firstOrNull { it.name == savedInstanceState?.getString(STATE_SIDE) },
    ...
    private companion object {
        ...
        val STAGE_TIPS = listOf(
            StageTip.TextTip(id = "tip_1", cost = 1, textRes = R.string.stage5_tip_1),
            StageTip.TextTip(id = "tip_2", cost = 1, textRes = R.string.stage5_tip_2),
        )
    }
}

@Composable
private fun Stage5Screen(
    playerInfo: PlayerInfo,
    counterValue: Int?,
    answer: String,
    ...
    StageLayout(
        ...
        tipsUsedForStage = playerInfo.tipsUsedForStage(5),
        ...
        BoxWithConstraints(modifier = Modifier.fillMaxSize()) {
            ...
            Box(modifier = Modifier.fillMaxSize().verticalScroll(rememberScrollState())) {
                Column(
                    ...
                    if (playerInfo.scoreForStage(5) > 0 && !answerIsCorrect) {
                        CompletedStageReview(correctAnswer = stringResource(R.string.stage5_answer)) {
                            Stage5CounterText(counterValue)
                        }
                    } else {
                        Box(
...
@Composable
private fun Stage5CounterText(value: Int?) {
